...
class UKF:
    ...
    def __init__(self, model, x_0, measurements, h, options, start_suffix='_0', pars_fixed=None):
        """Constructor
            ...
            start_suffix -- Suffix to derive the start parameter of a variable. 
                It can also be a method with rules to derive the start parameter values from state variable names (String or Method)
            pars_fixed -- Set of fixed parameters in the observer model
    ...
    def _reset_model(self):
        #=============================================================
        # For some reason the program crashes without throwing any error 
        # when trying to simulate after model.reset(), that's why I'm 
        # using model.instantiate()
        #=============================================================
        self.model.instantiate()
        
        if self.pars_fixed is not None:
            self.model.set(list(self.pars_fixed.keys()), list(self.pars_fixed.values()))
        return self.model
    ...
    def update(self,y):
        ...
        Calls _update.
            ...
            s -- Confidence interval estimate (non-scaled) ({string:float})
        ...
        for i,state in enumerate(self.x):      
            ...
            s_out[state.get_name()] = N.sqrt(self.P[i,i]*state.get_nominal_value()**2)
    ...
    def predict(self, u=(), known_values={}):
        ...
        [xp, yp, K, P] = self._predict(sigma, self.x, u, known_values, self.P_v, self.P_n, self.currTime, self.h, self.mes, self.Wm, self.Wc)
    ...
    def _predict(self, sigma, x, u, known_values, P_v, P_n, currTime, h, measurements, Wm, Wc):
        ...
        for i in range(0, sigma.shape[1]):
            ...
            while retry:
                ...
                for j, state in enumerate(x):
                    ...
                    if isinstance(self.start_suffix, str) : 
                        self.model.set(state.get_name()+self.start_suffix, sigma[j,i]*state.get_nominal_value())
                    else:
                        self.model.set(self.start_suffix(state.get_name()), sigma[j,i]*state.get_nominal_value())
                    
                #Set known values
                for known in known_values:
                    if isinstance(self.start_suffix, str):
                        self.model.set(known+self.start_suffix, known_values[known])
                    else:
                        self.model.set(self.start_suffix(known), known_values[known])
                    
                #Simulate and extract result
                opt = self.model.simulate_options()
                opt['CVode_options']['atol'] = 1e-8
                opt['CVode_options']['rtol'] = 1e-6
                print('Simulating sigma-point '+str(i+1)+' out of '+str(sigma.shape[1])+' :')
                try:
                    result = self.model.simulate(start_time = currTime, final_time = currTime + h, options = opt, input = u)
                except:
                    retry = True
